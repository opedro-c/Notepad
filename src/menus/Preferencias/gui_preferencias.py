...
def montar_preferencias(master: Toplevel, text: Text, preferencias: dict, aplicar_preferencias) -> None:
    ...
    tamanho.insert(0, preferencias['tamanho'])
    tamanho.place(x=20, y=80, width=40)

    Label(lb_frame, text='Estilo', bg='white', fg='black').place(x=130, y=10)
    estilos = ('', 'bold', 'italic')
    estilo = StringVar()
    estilo.set(preferencias['estilo'])
    estilo_opcao = IntVar()
    estilo_opcao.set(estilos.index(preferencias['estilo']))
    Radiobutton(
        lb_frame, text='Regular', bg='white', fg='black', 
        variable=estilo_opcao, value=0, 
        command=lambda: estilo.set(estilos[estilo_opcao.get()])
    ).place(x=130, y=30)
    Radiobutton(
        lb_frame, text='Negrito', bg='white', fg='black', 
        variable=estilo_opcao, value=1, 
        command=lambda: estilo.set(estilos[estilo_opcao.get()])
    ).place(x=130, y=55)
    Radiobutton(
        lb_frame, text='Itálico', bg='white', fg='black', 
        variable=estilo_opcao, value=2, 
        command=lambda: estilo.set(estilos[estilo_opcao.get()])
    ).place(x=130, y=80)
    ...
    quebra_de_linha.set(preferencias['quebra_linha'])
    quebra_de_linha_opcao = IntVar()
    quebra_de_linha_opcao.set(tipos_quebra_de_linha.index(preferencias['quebra_linha']))
    Radiobutton(
        master, text='Sem quebra de linha', bg='white', fg='black', borderwidth=0,
        variable=quebra_de_linha_opcao, value=0, 
        command=lambda: quebra_de_linha.set(tipos_quebra_de_linha[quebra_de_linha_opcao.get()])
    ).place(x=20, y=190)
    Radiobutton(
        master, text='Dividir palavra em duas linhas', bg='white', fg='black', borderwidth=0, 
        variable=quebra_de_linha_opcao, value=1, 
        command=lambda: quebra_de_linha.set(tipos_quebra_de_linha[quebra_de_linha_opcao.get()])
    ).place(x=20, y=210)
    Radiobutton(
        master, text='Quebra de linha com palavra inteira', bg='white', fg='black', borderwidth=0,
        variable=quebra_de_linha_opcao, value=2,
        command=lambda: quebra_de_linha.set(tipos_quebra_de_linha[quebra_de_linha_opcao.get()])
    ).place(x=20, y=230)
    ...
    recuo.delete(0, END)
    recuo.insert(0, preferencias['recuo'])
    recuo.place(x=230, y=290, width=40)

    Button(master, text='Fechar', command=master.destroy).place(x=10, y=360)
    Button(
        master, text='Aplicar', 
        command=lambda: aplicar_preferencias(
            text, fonte.get(), tamanho.get(), estilo.get(), 
            quebra_de_linha.get(), int(recuo.get())
        )
    ).place(x=220, y=360)
